...
def generateParable(s, computers, do):
    ...
    if (nounOrVerb == 0): # noun case
        ...
        s += " " + data[verb]
        # adverbs?
        doAdverb = random.randint(0, 1)
        if doAdverb == 1: # add an adverb
            adverbFile = open("adverbs.txt")
            adverbs = adverbFile.read()
            adverbs = adverbs.splitlines()
            adverbIndex = random.randint(0, len(adverbs))
            s += " " + adverbs[adverbIndex]

    return s

# 50% of all verb phrases have an adverb appended as well.
